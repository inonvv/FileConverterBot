...
async def handle_message2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        ...
        log_info("try to get file", {"file_id": file_id})
        try:
            file = await context.bot.get_file(file_id)
        except Exception as error:
            log_error("failed to get the file", {"error message": str(error)})
            await update.message.reply_text("fail to upload a file.")
            return

        original_file_path = f"./{document.file_name}"  # Save the file with its original name
        log_info("try to save file", {"file path": original_file_path})
        try:
            await file.download_to_drive(original_file_path)
            log_success("file saved successfully", {"file path": original_file_path})
            # Store the file path for the user
            user_files[update.effective_user.id] = original_file_path
        except Exception as error:
            log_error("failed to save", {"file path": original_file_path})
            await update.message.reply_text("failed to save file")
            return
        # Generate inline keyboard with extension options
        keyboard = [
            [InlineKeyboardButton(ext.name, callback_data=ext.name) for ext in Extension]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        # Ask the user to choose an extension
        await update.message.reply_text("Choose the extension to convert your file to:", reply_markup=reply_markup)
    else:
        await update.message.reply_text("Please send a document to convert.")
